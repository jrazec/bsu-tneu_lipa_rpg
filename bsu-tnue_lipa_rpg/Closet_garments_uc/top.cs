...
namespace bsu_tnue_lipa_rpg.Closet_garments_uc
{
    public partial class top : UserControl
    {
        ...
        private void top4_desc_Click(object sender, EventArgs e)
        {
           ...
           /* DialogResult buy = MessageBox.Show($"This item cost: {}", "Warning", MessageBoxButtons.YesNo);
            if (buy == DialogResult.Yes)
            {
                if(Bedroom.instance.CURRENT_MONEY < Closet.instance.ITEM_PRICE[0, 3])
                {
                    MessageBox.Show("Not Enough Pera :'<",,MessageBoxIcon.Exclamation);
                }
                else
                {
                    Bedroom
